...
export async function exportarCatalogoLicitacion(cotId: string, opts: OpcionesCatalogo): Promise<void> {
  ...
  for (const arr of grupos.values()) arr.sort((a, b) => (a.order_index ?? 0) - (b.order_index ?? 0) || String(a.name || '').localeCompare(String(b.name || '')))
  const nombresGrupo = Array.from(grupos.keys()).sort((a, b) => a.localeCompare(b, 'es'))

  const importe = (it: ItemDB) => (Number(it.price) || 0) * (Number(it.quantity) || 0)
  ...
  const notasPie = (): Fila[] => {
    const n: Fila[] = [
      ...
      { celdas: [`4. Vigencia de la propuesta: ${opts.vigenciaDias || 15} días naturales a partir de la fecha de emisión.`] },
    ]
    if (!opts.conPrecios) {
      n.push({ celdas: ['5. El licitante deberá llenar las columnas PRECIO UNITARIO e IMPORTE de cada concepto, sin modificar descripciones ni cantidades.'] })
      n.push({ celdas: ['6. La propuesta deberá presentarse firmada, indicando tiempo de entrega y condiciones de pago por partida.'] })
      n.push({ celdas: [] })
      ...
      arr.forEach((it, ii) => {
        filas.push({
          ...
          celdas: [
            ...
            it.description || '',
            it.marca || '',
            it.modelo || it.sku || it.nomenclatura || '',
            unidadDe(it),
            Number(it.quantity) || 0,
            opts.conPrecios ? (Number(it.price) || 0) : '',
  ...
  if (esESP && nombresGrupo.length > 1) {
    ...
    const resumen: Fila[] = nombresGrupo.map((g, i) => {
      ...
      return { estilo: 'dato' as const, celdas: [String(i + 1), g.toUpperCase(), `${arr.length} conceptos`, '', '', '', '', '', opts.conPrecios ? t : ''] }
    })
    const totalGral = nombresGrupo.reduce((s, g) => s + (grupos.get(g) || []).reduce((a, it) => a + importe(it), 0), 0)
    const hr: HojaXlsx = {
      nombre: nombreHoja('Resumen', usados),
      columnas: [
        { titulo: 'PARTIDA', ancho: 10 },
        { titulo: 'SISTEMA', ancho: 34 },
        { titulo: 'CONCEPTOS', ancho: 18 },
        { titulo: '', ancho: 4 }, { titulo: '', ancho: 4 }, { titulo: '', ancho: 4 }, { titulo: '', ancho: 4 },
        { titulo: '', ancho: 4 },
