...
export default function MisObrasPage({ employeeId }: { employeeId: string }) {
  ...
  const load = async () => {
    ...
    // 1. From weekly_plan_assignments
    const { data: planned } = await supabase
      .from('weekly_plan_assignments')
      .select('project_id, projects(id, name, direccion_completa, status)')
    ...
    // 2. From installer_attendance
    const { data: attendance } = await supabase
      .from('installer_attendance')
      .select('project_id, fecha, projects(id, name, direccion_completa, status)')
      .eq('employee_id', employeeId)
    ...
    // 3. From obra_reportes (via obras → project_id)
    const { data: reportes } = await supabase
      .from('obra_reportes')
      .select('obras(id, project_id, nombre, direccion, status)')
      .eq('instalador_id', employeeId)
    for (const r of (reportes || [])) {
      const o = (r.obras as any)
      if (!o || !o.project_id) continue
      if (!map.has(o.project_id)) {
        map.set(o.project_id, {
          project_id: o.project_id,
          project_name: o.nombre,
          ...
          {obras.map(o => (
            ...
            >
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 4 }}>{o.project_name}</div>
                {o.direccion && (
