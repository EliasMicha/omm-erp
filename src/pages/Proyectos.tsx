...
export default function Proyectos() {
  ...
  return (
    <div style={{ padding: '24px 28px' }}>
      <SectionHeader title="Proyectos" subtitle={`${projects.length} proyectos totales`}
        action={<Btn variant="primary"><Plus size={14} /> Nuevo proyecto</Btn>} />
      <div style={{ display: 'flex', gap: 6, marginBottom: 20 }}>
        {['todos','activo','pausado','completado','cancelado'].map(f => {
          const on=filtro===f;const cfg=f!=='todos'?STATUS_CONFIG[f as any]:null
          return(<button key={f} onClick={()=>setFiltro(f)} style={{padding:'5px 12px',borderRadius:20,fontSize:11,cursor:'pointer',fontFamily:'inherit',border:`1px solid ${on?(cfg?.color||'#57FF9A'):'#333'}`,background:on?(cfg?.color||'#57FF9A')+'22':'transparent',color:on?(cfg?.color||'#57FF9A'):'#666',}}>{f.charAt(0).toUpperCase()+f.slice(1)}</button>)
        })}
      </div>
              ...
              <div key={p.id} style={{background:'#141414',border:'1px solid #222',borderRadius:12,padding:'16px 18px',borderTop:`2px solid ${cfg.color}33`}}>
                ...
                <div style={{display:'flex',gap:4,flexWrap:'wrap',marginBottom:12}}>
                  {p.lines.map(l => {
                    ...
                    return <Badge key={l} label={e.icon+' '+e.label} color={e.color}/>
                  })}
                </div>
