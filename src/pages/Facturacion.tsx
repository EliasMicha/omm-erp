...
function DetalleModal(props: { factura: Factura; conceptos: any[]; loading: boolean; onClose: () => void; onPdf: () => void; onXml: () => void }) {
  ...
  const dirBg = isEmit ? 'rgba(87,255,154,0.15)' : 'rgba(251,191,36,0.12)'
  const dirColor = isEmit ? '#57FF9A' : '#fcd34d'
  const statusBg = f.status === 'timbrada' ? '#57FF9A22' : f.status === 'cancelada' ? '#EF444422' : '#F59E0B22'
  const statusColor = f.status === 'timbrada' ? '#57FF9A' : f.status === 'cancelada' ? '#EF4444' : '#F59E0B'
  const subtotalNum = Number(f.subtotal) || 0
  const totalNum = Number(f.total) || 0
  const ivaNum = Number(fAny.iva) || 0
  const fechaEmStr = f.fecha_emision ? new Date(f.fecha_emision).toLocaleString('es-MX') : '--'
  const fechaTimStr = fAny.fecha_timbrado ? new Date(fAny.fecha_timbrado).toLocaleString('es-MX') : '--'
      ...
      <div onClick={(e) => e.stopPropagation()} style={{ background: '#0a0a0a', border: '1px solid #2a2a2a', borderRadius: 12, maxWidth: 1000, width: '100%', padding: 24, color: '#ddd' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20, paddingBottom: 16, borderBottom: '1px solid #1e1e1e' }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
              <span style={{ padding: '3px 10px', borderRadius: 4, fontSize: 10, fontWeight: 700, letterSpacing: '0.05em', background: dirBg, color: dirColor }}>{dirLabel}</span>
              <span style={{ fontSize: 11, color: '#666' }}>Tipo: {f.tipo_comprobante || 'I'}</span>
              <span style={{ padding: '3px 10px', borderRadius: 10, fontSize: 10, fontWeight: 600, background: statusBg, color: statusColor }}>{f.status}</span>
            </div>
            <div style={{ fontSize: 18, fontWeight: 700, color: '#fff' }}>{f.serie || ''}{f.folio || '--'}</div>
            <div style={{ fontSize: 11, color: '#888', fontFamily: 'monospace', marginTop: 4 }}>{f.uuid_fiscal || 'Sin UUID'}</div>
          </div>
          <button onClick={props.onClose} style={{ background: 'none', border: 'none', color: '#666', cursor: 'pointer', padding: 4 }}><X size={20} /></button>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>
          <div>
            ...
            <div style={{ fontSize: 13, color: '#fff', fontWeight: 600 }}>{f.emisor_nombre || '--'}</div>
            <div style={{ fontSize: 11, color: '#888', fontFamily: 'monospace', marginTop: 2 }}>{f.emisor_rfc || '--'}</div>
            <div style={{ fontSize: 11, color: '#666', marginTop: 2 }}>Regimen: {fAny.emisor_regimen_fiscal || '--'}</div>
          </div>
          <div>
            ...
            <div style={{ fontSize: 13, color: '#fff', fontWeight: 600 }}>{f.receptor_nombre || '--'}</div>
            <div style={{ fontSize: 11, color: '#888', fontFamily: 'monospace', marginTop: 2 }}>{f.receptor_rfc || '--'}</div>
            <div style={{ fontSize: 11, color: '#666', marginTop: 2 }}>Regimen: {fAny.receptor_regimen_fiscal || '--'}</div>
            <div style={{ fontSize: 11, color: '#666', marginTop: 2 }}>CP: {fAny.receptor_codigo_postal || '--'}</div>
            <div style={{ fontSize: 11, color: '#666', marginTop: 2 }}>Uso CFDI: {fAny.receptor_uso_cfdi || '--'}</div>
          </div>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 20, padding: '12px 14px', background: '#0e0e0e', borderRadius: 8, border: '1px solid #1e1e1e' }}>
          <div>
            <div style={{ fontSize: 9, color: '#555', textTransform: 'uppercase' as const, marginBottom: 2 }}>Fecha emision</div>
            <div style={{ fontSize: 12, color: '#ccc' }}>{fechaEmStr}</div>
          </div>
          <div>
            <div style={{ fontSize: 9, color: '#555', textTransform: 'uppercase' as const, marginBottom: 2 }}>Fecha timbrado</div>
            <div style={{ fontSize: 12, color: '#ccc' }}>{fechaTimStr}</div>
          </div>
          <div>
            <div style={{ fontSize: 9, color: '#555', textTransform: 'uppercase' as const, marginBottom: 2 }}>Forma pago</div>
            <div style={{ fontSize: 12, color: '#ccc' }}>{fAny.forma_pago || '--'}</div>
          </div>
          <div>
            <div style={{ fontSize: 9, color: '#555', textTransform: 'uppercase' as const, marginBottom: 2 }}>Metodo pago</div>
            <div style={{ fontSize: 12, color: '#ccc' }}>{fAny.metodo_pago || '--'}</div>
          </div>
        </div>
        <div style={{ marginBottom: 16 }}>
          <div style={{ fontSize: 11, color: '#888', textTransform: 'uppercase' as const, letterSpacing: '0.06em', marginBottom: 8, fontWeight: 600 }}>Conceptos ({props.conceptos.length})</div>
          {props.loading ? (
            <div style={{ padding: 20, textAlign: 'center' as const, color: '#555', fontSize: 12 }}>Cargando conceptos...</div>
          ) : props.conceptos.length === 0 ? (
            <div style={{ padding: 20, textAlign: 'center' as const, color: '#555', fontSize: 12, background: '#0e0e0e', borderRadius: 8, border: '1px solid #1e1e1e' }}>Sin conceptos guardados.</div>
          ) : (
            <div style={{ background: '#0e0e0e', border: '1px solid #1e1e1e', borderRadius: 8, overflow: 'hidden' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' as const, fontSize: 11 }}>
                ...
                <tbody>
                  {props.conceptos.map((c: any, i: number) => {
                    const cant = Number(c.cantidad) || 0
                    const vUnit = Number(c.valor_unitario) || 0
                    const imp = Number(c.importe) || 0
                    const ivaImp = Number(c.iva_importe) || 0
                    const ivaStr = ivaImp > 0 ? '
// ============================================================
...
function ListaRecibidas() {
 ...
 + ivaImp.toLocaleString('es-MX', { minimumFractionDigits: 2 }) : '--'
                    return (
                      <tr key={c.id || i} style={{ borderBottom: '1px solid #1a1a1a' }}>
                        ...
                        <td style={{ padding: '8px 10px', color: '#ccc', textAlign: 'right' as const }}>{cant.toLocaleString('es-MX')}</td>
                        <td style={{ padding: '8px 10px', color: '#888' }}>{c.unidad || c.clave_unidad}</td>
                        <td style={{ padding: '8px 10px', color: '#ccc', textAlign: 'right' as const }}>${vUnit.toLocaleString('es-MX', { minimumFractionDigits: 2 })}</td>
                        <td style={{ padding: '8px 10px', color: '#fff', fontWeight: 600, textAlign: 'right' as const }}>${imp.toLocaleString('es-MX', { minimumFractionDigits: 2 })}</td>
                        <td style={{ padding: '8px 10px', color: '#888', textAlign: 'right' as const }}>{ivaStr}</td>
                      </tr>
                    )
                  })}
                </tbody>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 20 }}>
          <div style={{ minWidth: 280, background: '#0e0e0e', border: '1px solid #1e1e1e', borderRadius: 8, padding: 14 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 14, color: '#fff', fontWeight: 700, paddingTop: 8, borderTop: '1px solid #1e1e1e', marginTop: 4 }}>
              <span>Total:</span>
        ...
        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
          {hasFacturapiId ? (
            <button onClick={props.onPdf} style={{ padding: '8px 14px', background: '#1e1e1e', border: '1px solid #2a2a2a', borderRadius: 8, color: '#a78bfa', fontSize: 12, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, fontFamily: 'inherit' }}>
              <Download size={12} /> PDF
            </button>
          ) : null}
          {hasFacturapiId ? (
            <button onClick={props.onXml} style={{ padding: '8px 14px', background: '#1e1e1e', border: '1px solid #2a2a2a', borderRadius: 8, color: '#60a5fa', fontSize: 12, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, fontFamily: 'inherit' }}>
              <Download size={12} /> XML
            </button>
          ) : null}
          <button onClick={props.onClose} style={{ padding: '8px 14px', background: '#57FF9A', border: 'none', borderRadius: 8, color: '#000', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>Cerrar</button>
        </div>
      </div>
    </div>
  )
