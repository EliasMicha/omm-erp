  ...
  return (
    <div style={{ padding: '24px 28px' }}>
      <SectionHeader
        ...
        action={<Btn variant="primary" onClick={() => setShowNew(true)}><Plus size={14} /> Nueva cotización</Btn>}
      />
      ...
      {loading ? <Loading /> : (
        <Table>
          ...
          <tbody>
            {lista.length === 0 && (
              <tr><td colSpan={6}><EmptyState message="Sin cotizaciones — crea la primera" /></td></tr>
            )}
            {lista.map(c => {
              const esp = SPECIALTY_CONFIG[c.specialty]
              ...
              return (
                <tr key={c.id} style={{ cursor: 'pointer' }} onClick={() => onOpen(c.id)}>
                  ...
                  <Td muted>{proj?.name || '—'}<br /><span style={{ fontSize: 10 }}>{proj?.client_name}</span></Td>
      ...
      {showNew && <NuevaCotModal onClose={() => setShowNew(false)} onCreated={id => { setShowNew(false); onOpen(id) }} />}
    </div>
  )
}

// ── MODAL NUEVA COTIZACIÓN ────────────────────────────────────────────────────
function NuevaCotModal({ onClose, onCreated }: { onClose: () => void; onCreated: (id: string) => void }) {
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
      <div style={{ background: '#141414', border: '1px solid #333', borderRadius: 16, padding: 24, width: 480 }}>
        ...
        <div style={{ display: 'grid', gap: 12 }}>
          <label style={{ fontSize: 11, color: '#555', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
            ...
            <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
              placeholder="ej. Cero5cien O-402 — Especiales"
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 6 }}>
              {Object.entries(SPECIALTY_CONFIG).map(([k, v]) => (
                <button key={k} onClick={() => setForm(f => ({ ...f, specialty: k }))}
                  style={{ padding: '5px 12px', borderRadius: 20, fontSize: 11, cursor: 'pointer', fontFamily: 'inherit', fontWeight: 600, border: `1px solid ${form.specialty === k ? v.color : '#333'}`, background: form.specialty === k ? v.color + '22' : 'transparent', color: form.specialty === k ? v.color : '#666' }}>
                  {v.icon} {v.label}
                </button>
              ))}
            </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 20 }}>
          ...
          <Btn variant="primary" onClick={crear}>{saving ? 'Creando...' : 'Crear cotización →'}|/Btn>
        </div>
      </div>
    </div>
  )
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '175px 1fr', flex: 1, overflow: 'hidden' }}>
        ...
        <div style={{ borderRight: '1px solid #222', overflowY: 'auto', background: '#0e0e0e' }}>
          <div style={{ padding: '8px 8px 4px', fontSize: 9, fontWeight: 600, color: '#444', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Áreas</div>
          {areas.map(a => {
            ...
            return (
              <div key={a.id} onClick={() => setAreaActiva(a.id)} style={{
                ...
                fontSize: 11, color: active ? '#fff' : '#666',
          ...
          <div onClick={addArea} style={{ margin: '4px 8px', padding: '4px', border: '1px dashed #333', borderRadius: 6, textAlign: 'center', cursor: 'pointer', fontSize: 10, color: '#444' }}>+ Área</div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                ...
                {areaItems.map(item => (
                  <tr key={item.id}>
                    ...
                    <td style={{ padding: '7px 8px', borderBottom: '1px solid #1a1a1a' }}>
                      {item.system && <Badge label={item.system} color="#555" />}
                    </td>
                    ...
                    <td style={{ padding: '7px 8px', borderBottom: '1px solid #1a1a1a' }}>
                      <button onClick={() => removeItem(item.id)} style={{ background: 'none', border: 'none', color: '#444', cursor: 'pointer', fontSize: 16 }}>×</button>
                    </td>
                  </tr>
                ))}
                <tr>
          ...
          <div style={{ borderTop: '1px solid #222', padding: '10px 14px', display: 'flex', gap: 24, flexShrink: 0, background: '#0e0e0e', fontSize: 11 }}>
            {(['material','labor'] as const).map(tipo => {
              ...
              return (
                <div key={tipo}>
                  <span style={{ color: '#555', textTransform: 'capitalize' }}>{tipo === 'material' ? 'Equipo' : 'Labor'}: </span>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: '#141414', border: '1px solid #333', borderRadius: 16, padding: 20, width: 700, maxHeight: '80vh', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 14 }}>
              <div style={{ fontSize: 15, fontWeight: 600, color: '#fff' }}>Catálogo de productos</div>
            ...
            <div style={{ overflowY: 'auto', flex: 1 }}>
              <Table>
                <thead>
                  <tr>
                    <Th>Producto</Th><Th>Sistema</Th><Th>Tipo</Th><Th>Proveedor</Th><Th right>Precio</Th><Th></Th>
                  </tr>
                </thead>
                <tbody>
                  {catalog.length === 0 && <tr><td colSpan={6}><EmptyState message="Catálogo vacío — agrega productos en Supabase" /></td></tr>}
                  {catalog.map(p => (
                    <tr key={p.id}>
