...
function TabConciliacion({ bankMovements, setBankMovements, invoices, projectNames }: { bankMovements: BankMovement[]; setBankMovements: (m: BankMovement[]) => void; invoices: Invoice[]; projectNames: string[] }) {
  ...
  const updateAssignment = async (movId: string, field: 'lead_id' | 'quotation_id' | 'purchase_order_id', value: string | null) => {
    ...
    try {
      ...
      // Actualizar el state local — sync proyecto_sugerido alias
      const localUpdates = { ...updates }
      if ('proyecto' in localUpdates) localUpdates.proyecto_sugerido = localUpdates.proyecto
      setBankMovements(bankMovements.map(bm => bm.id === movId ? { ...bm, ...localUpdates } : bm))
    } finally {
                  ...
                  {isExpanded && (
                    <tr>
                      ...
                      {(() => {
                        ...
                        // Buscador siempre disponible (aunque haya match exacto, puede no ser el correcto)
                        ...
                        const searchedList = hasSearchActive ? candidateInvoices.filter(inv => {
                          const who = (inv.direccion === 'emitida' ? inv.receptor_nombre : inv.emisor_nombre) || ''
                        ...
                        // Lista que se renderiza (prioridad):
                        // 1. Si búsqueda activa → resultados filtrados (puede sobrescribir match exacto)
                        // 2. Si hay matches exactos sin búsqueda → mostrar los exactos como sugerencia
                        // 3. Si no hay nada → lista vacía con prompt para buscar
                        const sorted = hasSearchActive ? searchedList : (hasExactMatches ? exactMatches : [])
                        ...
                        return (
                          <div style={{ background: '#141414', border: '1px solid #1f1f1f', borderRadius: 6, padding: '8px 10px', marginBottom: 8 }}>
                            ...
                            {/* Add invoice - búsqueda con filtros (siempre disponible) */}
                            {!fullyLinked && (
                              <div>
                                {/* Status: si hay match exacto, mostrar info verde; si no, amarillo */}
                                {hasExactMatches && !hasSearchActive ? (
                                  <div style={{ fontSize: 10, color: '#22c55e', marginBottom: 6, fontWeight: 600 }}>
                                    ✓ Match exacto sugerido · Restante: <span style={{ fontFamily: 'monospace' }}>{F(remaining)}</span>
                                    <span style={{ color: '#666', fontWeight: 400, marginLeft: 6 }}>— si no es el correcto, usa los filtros para buscar otra</span>
                                  </div>
                                ) : (
                                  <div style={{ fontSize: 10, color: hasSearchActive ? '#888' : '#eab308', marginBottom: 6, fontWeight: 600 }}>
                                    {hasSearchActive ? '🔍 Búsqueda activa' : '⚠ Sin match exacto por monto'} · Restante: <span style={{ fontFamily: 'monospace' }}>{F(remaining)}</span>
                                    {!hasSearchActive && <span style={{ color: '#666', fontWeight: 400, marginLeft: 6 }}>— busca manualmente entre facturas no conciliadas</span>}
                                  </div>
                                )}
                                {/* Buscador con filtros SIEMPRE visible */}
                                ...
                                <div style={{ fontSize: 10, color: '#666', marginBottom: 4 }}>
                                  {hasSearchActive
                                    ? `${sorted.length} resultado${sorted.length !== 1 ? 's' : ''} de búsqueda`
                                    : hasExactMatches
                                      ? `${exactMatches.length} match${exactMatches.length !== 1 ? 'es' : ''} exacto${exactMatches.length !== 1 ? 's' : ''} por monto — o filtra arriba para otra opción`
                                      : 'Escribe en los filtros para buscar entre todas las facturas no conciliadas'}
                                </div>
                                <div style={{ maxHeight: 180, overflowY: 'auto', border: '1px solid #1f1f1f', borderRadius: 4, background: '#111' }}>
                                  {sorted.length === 0 ? (
                                    <div style={{ fontSize: 10, color: '#555', padding: 8, textAlign: 'center' }}>
                                      {hasSearchActive ? 'Sin coincidencias — ajusta los filtros' : 'Usa los filtros arriba para buscar'}
                                    </div>
