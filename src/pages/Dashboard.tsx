    ...
    <div style={{ padding: '24px 28px', maxWidth: 1200 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 24 }}>
        <div>
          ...
          <Table>
            <thead><tr><Th>Proyecto</Th><Th>Cliente</Th><Th>Avance</Th><Th>Estado</Th><Th right>Contrato</Th></tr></thead>
            <tbody>
              {projects.length === 0 && <tr><Td colSpan={5} muted style={{ textAlign: 'center' }}>Sin proyectos activos</Td></tr>}
              {projects.map(p => {
        ...
        <div>
          ...
          <Table>
            ...
            <tbody>
              {milestones.length === 0 && <tr><Td colSpan={4} muted style={{ textAlign: 'center' }}>Sin cobros pendientes</Td></tr>}
              {milestones.slice(0, 6).map(m => (
                <tr key={m.id}>
                  <Td><span style={{ fontWeight: 500, color: m.status === 'vencido' ? '#EF4444' : '#ccc' }}>{m.name}</span></Td>
                  <Td muted>{([m.project] as any)?.name || '-'}</Td>
                  <Td muted>{m.due_date ? new Date(m.due_date).toLocaleDateString('es-MX') : '-'}</Td>
